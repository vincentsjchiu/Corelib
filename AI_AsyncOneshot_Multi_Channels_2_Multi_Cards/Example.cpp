    ...
    try
    {
        ...
        for (int point = 0; point < SAMPLE_COUNT; point++)
        {
            for (int i = 0; i < enablechcount; i++)
            {
                ...
                {
                    fp[0] << *(scaledDataBuff + i * (SAMPLE_COUNT)+point) << "\n";
                }
                else
                {
                    fp[0] << *(scaledDataBuff + i * (SAMPLE_COUNT)+point) << ",";
                ...
                {           
                    ...
                    for (int point = 0; point < SAMPLE_COUNT; point++)
                    {
                        for (int i = 0; i < enablechcount; i++)
                        {
                            if(i== enablechcount-1)
                            {
                            fp[1] << *(scaledDataBuff + i * (SAMPLE_COUNT)+point) << "\n";
                            }
                            else
                            {
                            fp[1] << *(scaledDataBuff + i * (SAMPLE_COUNT)+point) << ",";
