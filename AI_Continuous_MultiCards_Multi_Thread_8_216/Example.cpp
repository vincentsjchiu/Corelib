...
void AI_Continuous_Raw_Data(I_AITask* aiTask, int devIndex);
...
void ThreadRoutine(int devIndex, const char* aliasName)
{
    ...
    if (aiTask != NULL)
        AI_Continuous_Raw_Data(aiTask, devIndex);
    else
...
void AI_Continuous_Raw_Data(I_AITask* aiTask, int devIndex)
{
    ...
    if (aiTask->isDataReady())
    {
        DataProcess(aiTask, devIndex);
    }
    if (aiTask->isBufferOverrun()) {
        cout << "Buffer is overrun!!\n";
        // aiTask->BufferOverrunReset();
        //break;
    }
