...
void AI_Continuous_Multi_Channels_dev0(I_AITask* aiTask);
void AI_Continuous_Multi_Channels_dev1(I_AITask* aiTask);
void AI_Continuous_Multi_Channels_dev2(I_AITask* aiTask);
...
int main(int argc, char* argv[])
{
    try
    {
       ...
       fp[2].open((std::string)"dev2_" + filename, ios::out); // append: ios::out|ios::app
        ...
        if (aiTask0 != NULL)
            AI_Continuous_Multi_Channels_dev0(aiTask0);
        else
        ...
        if (aiTask1 != NULL)
            AI_Continuous_Multi_Channels_dev1(aiTask1);
        else
            cout << "The task is not support." << "\n";
        aiTask2 = dev2->getAITask();
        if (aiTask1 != NULL)
            AI_Continuous_Multi_Channels_dev2(aiTask2);
        else
            cout << "The task is not support." << "\n";
